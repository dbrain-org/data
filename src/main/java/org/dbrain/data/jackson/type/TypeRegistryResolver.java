...
public class TypeRegistryResolver implements TypeResolverBuilder<TypeRegistryResolver> {
  ...
  public TypeDeserializer buildTypeDeserializer(DeserializationConfig config, JavaType baseType, Collection<NamedType> subtypes) {
    ...
    if (entry != null) {
      TypeRegistryIdResolver idRes = new TypeRegistryIdResolver(entry.getRegistry());
      JavaType defaultImplJavaType = getDefaultImpl() !=  null ? config.getTypeFactory().constructType(getDefaultImpl()) : null;
      // First, method for converting type info to type id:
      switch (entry.getInsertAs()) {
        case WRAPPER_ARRAY:
          return new AsArrayTypeDeserializer(baseType, idRes, entry.getName(), entry.isVisible(), defaultImplJavaType);
        case PROPERTY:
        case EXISTING_PROPERTY: // as per [#528] same class as PROPERTY
          return new AsPropertyTypeDeserializer(baseType, idRes, entry.getName(), entry.isVisible(), defaultImplJavaType, entry.getInsertAs());
        case WRAPPER_OBJECT:
          return new AsWrapperTypeDeserializer(baseType, idRes, entry.getName(), entry.isVisible(), defaultImplJavaType);
        case EXTERNAL_PROPERTY:
          return new AsExternalTypeDeserializer(baseType, idRes, entry.getName(), entry.isVisible(), defaultImplJavaType);
        default:
          throw new IllegalStateException();
      }
    } else {
